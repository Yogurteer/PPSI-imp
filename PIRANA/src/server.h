...
class Server {
 ...
 public:
  ...
  Server(PirParms &pir_parms, bool is_batch, bool random_db, std::vector<std::vector<uint64_t>> input_db);
  void gen_random_db();
  bool read_db_from_file(const std::string& filename);
  
  // 直接设置数据库 (for LPSI protocol integration)
  void set_database(const std::vector<std::vector<uint64_t>>& db) {
    _raw_db = db;
    _set_db = true;
  }
